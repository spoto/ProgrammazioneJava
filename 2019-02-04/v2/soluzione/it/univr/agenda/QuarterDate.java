...
public class QuarterDate extends Date {
	...
	private QuarterDate(int daysAfterStartOf1900) {
	...
	private static int daysAfterStartOf1900(int day, Quarter quarter, int year) {
		if (year < 1900 || day < 1 || day > daysInQuarter(quarter, year))
			throw new IllegalArgumentException();

		int result = day - 1;
		for (Quarter previousQuarter = Quarter.Q1; previousQuarter.compareTo(quarter) < 0; previousQuarter = Quarter.values()[previousQuarter.ordinal() + 1])
			result += daysInQuarter(previousQuarter, year);

		for (int previousYear = 1900; previousYear < year; previousYear++)
			result += daysInYear(previousYear);
	...
	public String toString() {
		return String.format("%02d%s %d", getDay(), getQuarter(), getYear());
	}
	...
	public QuarterDate after(int days) {
		...
		return new QuarterDate(daysAfterStartOf1900 + days);
	}
}
