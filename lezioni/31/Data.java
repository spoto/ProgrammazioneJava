...
public class Data implements Ordinabile {
	...
	public boolean precede(Ordinabile altra) {
		if (!(altra instanceof Data))
			return false;

		Data altraComeData = (Data) altra;

		if (anno < altraComeData.anno)
			return true;
		else if (anno == altraComeData.anno)
			if (mese < altraComeData.mese)
				return true;
			else if (mese == altraComeData.mese)
				if (giorno < altraComeData.giorno)
					return true;
